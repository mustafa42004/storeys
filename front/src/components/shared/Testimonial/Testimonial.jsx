...
const Testimonial = () => {
  const slider = useRef(null);
    ...
    const TestimonialSlide = ({ name, role, comment, image }) => {
        return (
            <div className="slide">
                <img src={image} alt="" className="profile" />
                <div className="content">
                    <h2 className="font-lg text-left fs-30">{name}</h2>
                    <h6 className="font-sm text-left fs-26 medium">{role}</h6>
                    <img className="w-20" src="/assets/img/rates.svg" alt="" />
                    <p className="font-sm fs-22 text-left">{comment}</p>
